...
class BaseAgent(ABC):
    ...
    _JSON_OBJECT_REPAIR_PROMPT = PromptTemplate.from_template(
        "Extract or repair a JSON object from the following LLM output.\n"
        "The output MUST be a JSON object (not null, not array, not string).\n\n{input}"
    )
    ...
    async def _parse_json(self, text: str) -> dict:
        ...
        sanitized = self._THINK_BLOCK_PATTERN.sub("", text)
        sanitized = sanitized.replace("<think>", "").replace("</think>", "").strip()

        # Fast local repair path first: avoids extra model call for common malformed JSON.
        candidates = [sanitized]
        m = self._JSON_OBJECT_PATTERN.search(sanitized)
        if m:
        ...
        prompt_value = self._JSON_PARSE_PROMPT.format_prompt(input=sanitized)
        parsed = await self._json_output_parser.aparse_with_prompt(sanitized, prompt_value)
        if isinstance(parsed, dict):
            return parsed

        # One strict retry that enforces object-only output to avoid None/array leak-through.
        strict_prompt = self._JSON_OBJECT_REPAIR_PROMPT.format_prompt(input=sanitized)
        parsed = await self._json_output_parser.aparse_with_prompt(sanitized, strict_prompt)
        if isinstance(parsed, dict):
            return parsed
        raise ValueError(f"Invalid json output: expected object, got {type(parsed).__name__}")
