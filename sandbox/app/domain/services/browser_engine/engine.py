...
class BrowserEngine:
    ...
    def __init__(self, browser: Browser, model_kwargs: Optional[Dict[str, Any]] = None):
        self._browser = browser
        self._vision_enabled = bool(getattr(settings, "BROWSER_ENGINE_VISION_ENABLED", True))
        self._vision_runtime_enabled = self._vision_enabled
        self._vision_max_image_bytes = max(
            100_000,
            int(getattr(settings, "BROWSER_ENGINE_VISION_MAX_IMAGE_BYTES", 350_000) or 350_000),
        )
        self._vision_round_limit = max(
            1,
            min(int(getattr(settings, "BROWSER_ENGINE_VISION_ROUND_LIMIT", 6) or 6), 50),
        )
        ...
        kwargs.setdefault("model", getattr(settings, "MODEL_NAME", "gpt-4o"))
        kwargs.setdefault("model_provider", getattr(settings, "MODEL_PROVIDER", "openai"))
        kwargs.setdefault("temperature", min(float(getattr(settings, "TEMPERATURE", 0.7)), 0.4))
        kwargs.setdefault("max_tokens", min(int(getattr(settings, "MAX_TOKENS", 2000)), 1200))
