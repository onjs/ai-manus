...
5. 镜像与服务拆分策略落地：
   ...
   - `worker` 与 `worker-beat` 使用不同启动命令
   - `worker-beat` 默认仅 1 副本

## M1.1 Loop 参数传递对齐
1. Beat 触发任务时附带 `loop_config_snapshot` 到 `TriggerPayload`
2. Worker 启动 run 时将快照写入 `run_meta.loop.config_snapshot`
3. 保障快照与系统冻结参数一致，不允许运行中漂移
...
## M3 并发与配额控制
1. 全局/租户/Agent 并发上限
2. Worker 执行前申请并发令牌（原子 Lua/事务），失败回写 `pending`
3. `waiting` 会话不占 worker 执行槽位
4. 令牌释放补偿（完成/取消/超时/对账回收）

## M4 取消、超时、恢复
1. 取消 `pending|queued|running|waiting` 任务并收敛会话状态
2. 任务超时（`run_timeout_seconds`）后强制结束并释放并发令牌
3. 启动恢复扫描：对账 trigger/session 与 Celery 任务状态
4. Reconciler 规则落地：
   - `pending` 无 task 重新投递
...
3. 指标：`celery_queue_depth/celery_worker_alive/beat_lag/run_timeout/retry_exhausted`
...
7. 对账测试：人为制造丢任务/失联 worker，Reconciler 自动修正状态并补偿释放令牌
8. Beat HA 测试：多实例下单周期仅触发一次，leader 切换后可继续触发
9. 镜像复用测试：同一镜像拉起 `worker` 与 `worker-beat`，角色行为正确且日志可区分
10. 在线可见性测试：用户仅保持左侧常驻摘要流，不发 chat 也能实时看到自动任务创建与状态变化
